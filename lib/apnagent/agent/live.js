...
Agent.prototype.connect = function (cb) {
  ...
  var self = this
    ...
    , timeout = this.get('connect timeout')
    , gateway;
  ...
  if (!opts.pfx && !opts.key && !opts.cert) {
    process.nextTick(function () {
      ...
      self.debug('(gateway) error: %s', err.message);
  ...
  clearTimeout(this.meta.connectionTimer);

  this.meta.connectionTimer = setTimeout(function () {
    self.debug('(gateway) timeout');
    if (self.gateway) self.gateway.destroy();
    self.emit("gateway:timeout")
  }, ms(timeout));

  // connect to gateway
  self.debug('(gateway) connecting - %s:%d', opts.host, opts.port);
  gateway = tls.connect(opts, function () {
    clearTimeout(self.meta.connectionTimer);

    if (gateway.authorized) {
      self.debug('(gateway) connected - %s:%d', opts.host, opts.port);
      self.connected = true;
      self.cache.ttl = ttl;
      self.cache.resume();
      self.queue.resume();
      ...
      self.debug('(gateway) unauthorized - %s:%d', opts.host, opts.port, gateway.authorizationError);
  ...
  gateway.on('close', function () {
    ...
    if (!self.closing && recon) {
      self.emit("gateway:disconnect");
      self.debug('(gateway) disconnected - %s:%d', opts.host, opts.port);
    } else {
      self.debug('(gateway) closed - %s:%d', opts.host, opts.port);
      self.closing = false;
  ...
  gateway.on('data', function (buf) {
    self.debug('(gateway) data: %d bytes', buf.length);
    self.decoder.write(buf);
  });

  // emit errors;
  gateway.on('error', function (err) {
    self.debug('(gateway) error: %s', err.message || 'Unspecified Error');
    if (self.gateway) self.gateway.destroy();
    self.emit('gateway:error', err);
  });
...
Agent.prototype.close = function (cb) {
  ...
  this.queue.drain = function () {
    self.debug('(gateway) disconnecting');
    self.cache.pause();
...
Agent.prototype._queueIterator = function (obj, next) {
  ...
  encoder.once('readable', function () {
    ...
    if (!gateway || !gateway.writable || !self.connected) {
      self.debug('(queue) pause: not connected');
      queue.pause();
      queue.pushAt(0, obj);
      return next();
    }

    self.debug('(gateway) write: %d', id);
    gateway.write(buf, function () {
      self.debug('(cache) push: %d', id);
      cache.push(id, obj);
      next();
    });
  });
  ...
  // encode message
  self.debug('(encoder) write: %d', id);
  stream.write(obj.json);
};

Agent.prototype.debug = util.namespacedDebug("apnagent:agent-live");
