...
function Base () {
  ...
  this.set('connect timeout', '2s');
  this.set('debug id', null);
  ...
  queue.onerror = function (err) {
    self.debug('(queue) error: %s', err.message);
    self.emit('queue:error', err);
  };

  // emit queue drain on agent
  queue.drain = function () {
    self.debug('(queue) drain');
  ...
  decoder.stream(8).on('readable', function () {
    ...
    self.debug('(message) error: %s', err.message);
...
Base.prototype.send = function (msg, cb) {
  ...
  this.debug('(queue) push: %d', json.identifier);
...
Base.prototype.debug = util.namespacedDebug("apnagent:agent-base");
